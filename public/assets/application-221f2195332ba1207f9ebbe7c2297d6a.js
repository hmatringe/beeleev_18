...
(function() {
  ...
  CityAutocomplete = (function() {
    function CityAutocomplete(input, options) {
      this.city = null;
      this.input = input;
      this.options = options;
      this.autocomplete = new google.maps.places.Autocomplete(this.input[0], this.options);
      this.autocomplete.addListener('place_changed', this.placeChanged.bind(this));
    }

    CityAutocomplete.prototype.placeChanged = function() {
      this.place = this.autocomplete.getPlace();
      return this.setCityValue();
    };

    CityAutocomplete.prototype.setCityValue = function() {
      if (this.place.address_components != null) {
        this.city = this.place.address_components[0].long_name;
      } else {
        this.city = null;
      }
      return this.input.val(this.city);
    };
  ...
  $(document).on('ready page:load', function() {
    var ca;
    if ($('#user_city').size() === 1) {
      return ca = new CityAutocomplete($('#user_city'), {
        types: ['(cities)']
      });
    }
  });
...
(function() {
  var togglePendingConnectionDemandModal;

  $(function() {
    return $('.toggle-pending-connection-demand-modal').on('click', togglePendingConnectionDemandModal);
  });

  togglePendingConnectionDemandModal = function() {
...
(function() {
  $(document).on('ready page:load', function() {
    return $('.modal-content').on('click', '.connection-reject-link', function() {
      var rejectReasonIsPresent, textarea;
      $(this).parents('form').find('.connection-accept-link').attr('disabled', 'disabled');
      textarea = $(this).parents('form').find('textarea.connection-reject-description');
...
(function() {
  $(function() {
    return $('.btn-check-coupon').click(function() {
      return $(this).parents('.panel-footer').find('.input-group-addon').removeClass('glyphicon glyphicon-ok glyphicon-remove').end().find('.fa-spinner').show().css('visibility', 'visible');
    });
  });
...
(function() {
  $(document).on('ready page:load', function() {
    return Stripe.setPublishableKey('pk_test_EOW0PCgq1XgA0ixfdIsSHJX0');
  });

}).call(this);
(function() {
  var toggleUserContactInfoModal;

  $(document).on('ready page:load', function() {
    return $('.toggle-user-contact-info-modal').on('click', toggleUserContactInfoModal);
  });

  toggleUserContactInfoModal = function() {
    return false;
  };

}).call(this);
(function() {
  var connectionIdForAnswerConnectionModal, connectionIdForFeedbackModal, openAnswerConnectionModal, openFeedbackModal, openNewConnectionRequestModal, setupNetworkSearchSelect2, shouldOpenNewConnectionRequestModal;

  setupNetworkSearchSelect2 = function() {
    return $('#q_country_in, #q_city_eq:not([readonly]), #q_business_sectors_overlap, #q_international_activity_countries_overlap').select2({
      width: 'element'
    });
  };

  shouldOpenNewConnectionRequestModal = function() {
    return window.location.hash === "#new_connection_request";
  };

  openNewConnectionRequestModal = function() {
    return $("#navbar-new-connection-request-btn").click();
  };

  connectionIdForFeedbackModal = function() {
    var TypeError, connectionId, result, _ref;
    try {
      _ref = window.location.hash.match(/^#feedback-for-connection-(\d+)/), result = _ref[0], connectionId = _ref[1];
      return connectionId;
    } catch (_error) {
      TypeError = _error;
      return null;
  ...
  connectionIdForAnswerConnectionModal = function() {
    var TypeError, connectionId, result, _ref;
    try {
      _ref = window.location.hash.match(/^#answer-for-connection-(\d+)/), result = _ref[0], connectionId = _ref[1];
      return connectionId;
    } catch (_error) {
      TypeError = _error;
      return null;
  ...
  openFeedbackModal = function(connectionId) {
    return $("[href='/feedbacks/new?connection_id=" + connectionId + "']").click();
  };

  openAnswerConnectionModal = function(connectionId) {
    return $("[href^='/connection_'][href$='" + connectionId + "']").click();
  };

  $(document).on('ready page:load', function() {
    if (shouldOpenNewConnectionRequestModal()) {
    ...
    $(".modal-ajax").on('hidden.bs.modal', function(e) {
      ...
      if (href = $(e.target).data('remote')) {
        ...
        return $(e.target).data('remote', null);
      }
    });
    $(document).on('click', '[data-toggle="reopen-modal"]', function() {
      $('.modal-ajax').data('remote', $(this).attr('href'));
      $('.modal-ajax').modal('hide');
      return false;
    });
    $('#q_city_eq').on('change', function() {
      return this.form.submit();
    });
    $('#user_country').select2();
    $('#user_business_sector, #user_expertises, #user_spoken_languages, #user_international_activity_countries, #user_business_sectors, #user_investment_levels').select2();
    setupNetworkSearchSelect2();
    return $(window).resize(function() {
      return setupNetworkSearchSelect2();
    });
  });
